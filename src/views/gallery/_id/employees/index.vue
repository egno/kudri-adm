      ...
      <v-layout wrap>
        ...
        >
          <card
            ...
            :hover-effect="true"
            aspect-ratio="1"
            @titleClick="$router.push({name:'employeeGallery', params:{ id: businessId, personalId: emp.id }})"
            @imgClick="$router.push({name:'employeeGallery', params:{ id: businessId, personalId: emp.id }})"
          />
          <card
            v-else
            class="empty-photo"
          ...
          >
            <template slot="card-placeholder">
              <v-flex class="add-photo">
                <add-cover text="Добавить фотографию мастера" @filesUploaded="saveMasterPhoto($event, emp.id)" />
              </v-flex>
            </template>
          </card>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      saveMasterPhoto ({ fileNames }, employeeId) {
        ...
        this.changingEmployee.j =  { ...this.changingEmployee.j, ...{ image: fileNames[0].path } }
